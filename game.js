...
(() => {
  ...
  window.DOMHandler = class {
    ...
    _StartTicking() {
      this._hasTickCallback || (this._iRuntime._AddRAFCallback(this._tickCallback), this._hasTickCallback = true);
    }
    _StopTicking() {
      this._hasTickCallback && (this._iRuntime._RemoveRAFCallback(this._tickCallback), this._hasTickCallback = false);
    }
    Tick() {
    }
  };
  window.RateLimiter = class {
    constructor(d, a) {
      ...
      this._canRunImmediate = this._ignoreReset = false;
    }
    ...
    Call() {
      if (-1 === this._timerId) {
        var d = Date.now(), a = d - this._lastCallTime, b = this._interval;
  ...
  window.DOMElementHandler = class extends self.DOMHandler {
    ...
    _OnCreate(d) {
      const a = d.elementId, b = this.CreateElement(a, d);
    ...
    _OnSetVisible(d) {
      this._autoAttach && (this._elementMap.get(d.elementId).style.display = d.isVisible ? "" : "none");
    }
    _OnUpdatePosition(d) {
      if (this._autoAttach) {
...
self["${l}"]();`;
    ...
    const d = /(iphone|ipod|ipad|macos|macintosh|mac os x)/i.test(navigator.userAgent), a = /android/i.test(navigator.userAgent);
    let b = 0;
    let k = false, m = false;
    let u = new Audio();
    const z = { "audio/webm; codecs=opus": !!u.canPlayType("audio/webm; codecs=opus"), "audio/ogg; codecs=opus": !!u.canPlayType("audio/ogg; codecs=opus"), "audio/webm; codecs=vorbis": !!u.canPlayType("audio/webm; codecs=vorbis"), "audio/ogg; codecs=vorbis": !!u.canPlayType("audio/ogg; codecs=vorbis"), "audio/mp4": !!u.canPlayType("audio/mp4"), "audio/mpeg": !!u.canPlayType("audio/mpeg") };
    u = null;
    async function B(g) {
      g = await v(g);
    ...
    const C = [], F = /* @__PURE__ */ new Map(), y = /* @__PURE__ */ new Map();
    let G = 0;
    const D = [];
    ...
    const I = /* @__PURE__ */ new Set(["cordova", "playable-ad", "instant-games"]);
    let f = false;
    window.RuntimeInterface = class g {
      constructor(c) {
        ...
        !this._useWorker || "undefined" !== typeof OffscreenCanvas && navigator.userActivation && p() || (this._useWorker = false);
        if ("playable-ad" === this._exportType || "instant-games" === this._exportType) this._useWorker = false;
        if ("cordova" === this._exportType && this._useWorker) if (a) {
          const h = /Chrome\/(\d+)/i.exec(navigator.userAgent);
          h && 90 <= parseInt(h[1], 10) || (this._useWorker = false);
        ...
        "cordova" === this._exportType ? document.addEventListener(
          "deviceready",
          () => this._Init(c)
        ) : this._Init(c);
      }
      ...
      async _Init(c) {
        ...
        if ("playable-ad" === this._exportType) {
          ...
          for (let l = 0, n = c.engineScripts.length; l < n; ++l) {
            ...
            this._localFileStrings.hasOwnProperty(h) ? c.engineScripts[l] = { isStringSrc: true, str: this._localFileStrings[h] } : this._localFileBlobs.hasOwnProperty(h) && (c.engineScripts[l] = URL.createObjectURL(this._localFileBlobs[h]));
          }
        }
      ...
      async CreateWorker(c, h, l) {
        ...
        if ("cordova" === this._exportType && this._isFileProtocol) return c = await this.CordovaFetchLocalFileAsArrayBuffer(l.isC3MainWorker ? c : this._scriptFolder + c), c = new Blob([c], { type: "application/javascript" }), new Worker(URL.createObjectURL(c), l);
        c = new URL(c, h);
      ...
      async _InitWorker(c, h) {
        ...
        this._worker = await this.CreateWorker(l, this._baseUrl, { type: "module", name: "Runtime", isC3MainWorker: true });
        this._canvas = document.createElement("canvas");
        this._canvas.style.display = "none";
        l = this._canvas.transferControlToOffscreen();
        document.body.appendChild(this._canvas);
        window.c3canvas = this._canvas;
        let n = c.workerDependencyScripts || [], r = c.engineScripts;
        n = await Promise.all(n.map((q) => this._MaybeGetCordovaScriptURL(q)));
        r = await Promise.all(r.map((q) => this._MaybeGetCordovaScriptURL(q)));
        if ("cordova" === this._exportType) for (let q = 0, t = c.projectScripts.length; q < t; ++q) {
          const x = c.projectScripts[q], E = x[0];
          if (E === c.mainProjectScript || "scriptsInEvents.js" === E || E.endsWith("/scriptsInEvents.js")) x[1] = await this._MaybeGetCordovaScriptURL(E);
        }
        this._worker.postMessage(Object.assign(this._GetCommonRuntimeOptions(c), { type: "init-runtime", isInWorker: true, messagePort: h, canvas: l, workerDependencyScripts: n, engineScripts: r, projectScripts: c.projectScripts, mainProjectScript: c.mainProjectScript, projectScriptsStatus: self.C3_ProjectScriptsStatus }), [
          h,
          l,
          ...this._jobScheduler.GetPortTransferables()
        ]);
      ...
      async _InitDOM(c, h) {
        ...
        const n = c.mainProjectScript, r = c.projectScripts;
        for (let [q, t] of r) if (t || (t = q), q === n) try {
      ...
      PostToRuntimeComponent(c, h, l, n, r) {
        this._messageChannelPort.postMessage({ type: "event", component: c, handler: h, dispatchOpts: n || null, data: l, responseId: null }, r);
      }
      PostToRuntimeComponentAsync(c, h, l, n, r) {
        const q = G++, t = new Promise((x, E) => {
          y.set(q, { resolve: x, reject: E });
        });
        this._messageChannelPort.postMessage({ type: "event", component: c, handler: h, dispatchOpts: n || null, data: l, responseId: q }, r);
        return t;
      }
      ...
      _OnEventFromRuntime(c) {
        const h = c.component, l = c.handler, n = c.data, r = c.responseId;
        if (c = F.get(h)) if (c = c.get(l)) {
          var q = null;
          try {
            q = c(n);
          } catch (t) {
            console.error(`Exception in '${h}' handler '${l}':`, t);
            null !== r && this._PostResultToRuntime(r, false, "" + t);
            return;
          }
          if (null === r) return q;
          q && q.then ? q.then((t) => this._PostResultToRuntime(r, true, t)).catch((t) => {
            console.error(`Rejection from '${h}' handler '${l}':`, t);
            this._PostResultToRuntime(r, false, "" + t);
          }) : this._PostResultToRuntime(
            r,
            true,
            q
          );
      ...
      _PostResultToRuntime(c, h, l) {
        ...
        this._messageChannelPort.postMessage({ type: "result", responseId: c, isOk: h, result: l }, n);
      }
      _OnResultFromRuntime(c) {
        const h = c.responseId, l = c.isOk;
        c = c.result;
        const n = y.get(h);
      ...
      AddRuntimeComponentMessageHandler(c, h, l) {
        let n = F.get(c);
        n || (n = /* @__PURE__ */ new Map(), F.set(c, n));
      ...
      _FindRuntimeDOMHandler() {
        for (const c of this._domHandlers) if ("runtime" === c.GetComponentID()) {
      ...
      async _WasmDecodeWebMOpus(c) {
        c = await this.PostToRuntimeComponentAsync("runtime", "opus-decode", { arrayBuffer: c }, null, [c]);
      ...
      async _MaybeGetCordovaScriptURL(c) {
        return "cordova" === this._exportType && (c.startsWith("file:") || this._isFileProtocol && this.IsRelativeURL(c)) ? (c.startsWith(this._baseUrl) && (c = c.substr(this._baseUrl.length)), c = await this.CordovaFetchLocalFileAsArrayBuffer(c), c = new Blob([c], { type: "application/javascript" }), URL.createObjectURL(c)) : c;
      }
      ...
      async RequestPermissions(c) {
        if ("cordova" !== this.GetExportType() || this.IsiOSCordova()) return true;
        for (const h of c) if (!await this._HasPermission(h) && false === await this._RequestPermission(h)) return false;
        return true;
      }
      async RequirePermissions(...c) {
        if (false === await this.RequestPermissions(c)) throw Error("Permission not granted");
      }
      ...
      CordovaFetchLocalFileAsArrayBuffer(c) {
        return new Promise((h, l) => {
          w.push({ filename: c, successCallback: (n) => {
            A--;
      ...
      async _CordovaDoFetchLocalFileAsAsArrayBuffer(c, h, l) {
        try {
          const n = await this.CordovaFetchLocalFile(c), r = await v(n);
          h(r);
        } catch (n) {
          l(n);
        }
      ...
      async _ConvertDataUrisToBlobs() {
        const c = [];
        for (const [h, l] of Object.entries(this._localFileBlobs)) c.push(this._ConvertDataUriToBlobs(
          h,
          l
        ));
        await Promise.all(c);
      ...
      async _ConvertDataUriToBlobs(c, h) {
        if ("object" === typeof h) this._localFileBlobs[c] = new Blob([h.str], { type: h.type }), this._localFileStrings[c] = h.str;
        else {
      ...
      _BinaryStringToBlob(c, h) {
        var l = c.length;
        let n = l >> 2, r = new Uint8Array(l), q = new Uint32Array(r.buffer, 0, n), t, x;
        for (x = t = 0; t < n; ++t) q[t] = c.charCodeAt(x++) | c.charCodeAt(x++) << 8 | c.charCodeAt(x++) << 16 | c.charCodeAt(x++) << 24;
  ...
  }
  {
    let a = function(f) {
      ...
      do {
        if (f.parentNode && f.hasAttribute("contenteditable")) return true;
        f = f.parentNode;
      } while (f);
      return false;
    }, C = function(f) {
      ...
      if (!f) return false;
      const g = f.tagName.toLowerCase(), c = new Set("email number password search tel text url".split(" "));
      return "textarea" === g ? true : "input" === g ? c.has(f.type.toLowerCase() || "text") : w(f);
    };
    const d = self.RuntimeInterface;
    const b = /* @__PURE__ */ new Map([["OSLeft", "MetaLeft"], ["OSRight", "MetaRight"]]), e = { dispatchRuntimeEvent: true, dispatchUserScriptEvent: true }, k = { dispatchUserScriptEvent: true }, m = { dispatchRuntimeEvent: true };
    async function z(f) {
    ...
    async function v(f, g, c) {
      ...
      var h = await B(f);
      h = new DOMParser().parseFromString(h, "image/svg+xml");
      const l = h.documentElement;
      if (l.hasAttribute("width") && l.hasAttribute("height")) {
        const n = l.getAttribute("width"), r = l.getAttribute("height");
      ...
      f = new Blob([h], { type: "image/svg+xml" });
      return await z(f);
    }
    const A = /* @__PURE__ */ new Set(["input", "textarea", "datalist", "select"]);
    const F = /* @__PURE__ */ new Set(["canvas", "body", "html"]);
    ...
    let D = false;
    document.addEventListener("pause", () => D = true);
    document.addEventListener("resume", () => D = false);
    d.AddDOMHandlerClass(class extends self.DOMHandler {
      constructor(f) {
        ...
        this._attachedDeviceMotionEvent = this._attachedDeviceOrientationEvent = false;
        ...
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "raster-svg-image",
          (h) => this._OnRasterSvgImage(h)
        );
        ...
        window.addEventListener("contextmenu", (h) => {
          const l = h.target, n = l.tagName.toLowerCase();
        ...
        window.addEventListener("touchstart", y, { passive: false });
        "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", y, { passive: false }), c.addEventListener("pointerdown", y)) : c.addEventListener("touchstart", y);
        this._mousePointerLastButtons = 0;
        window.addEventListener("mousedown", (h) => {
          1 === h.button && h.preventDefault();
        });
        window.addEventListener("mousewheel", G, { passive: false });
        window.addEventListener("wheel", G, { passive: false });
        ...
        if (f.IsiOSWebView()) if (window.visualViewport) {
          let h = Infinity;
        ...
        this._mediaPendingPlay = /* @__PURE__ */ new Set();
        this._mediaRemovedPendingPlay = /* @__PURE__ */ new WeakSet();
        this._isSilent = false;
      }
      _OnBeforeStartTicking() {
        "cordova" === this._iRuntime.GetExportType() ? (document.addEventListener("pause", () => this._OnVisibilityChange(true)), document.addEventListener("resume", () => this._OnVisibilityChange(false))) : document.addEventListener("visibilitychange", () => this._OnVisibilityChange(document.hidden));
      ...
      Attach() {
        ...
        window.addEventListener("dblclick", (g) => this._OnMouseEvent(
          "dblclick",
          g,
          e
        ));
        ...
        }), this._iRuntime.UsesWorker() && "undefined" !== typeof window.onpointerrawupdate && self === self.top ? (this._pointerRawUpdateRateLimiter = new self.RateLimiter(() => this._DoSendPointerRawUpdate(), 5), this._pointerRawUpdateRateLimiter.SetCanRunImmediate(true), window.addEventListener(
          "pointerrawupdate",
          (g) => this._OnPointerRawUpdate(g)
        )) : window.addEventListener("pointermove", (g) => this._OnPointerEvent("pointermove", g)), window.addEventListener("pointerup", (g) => this._OnPointerEvent("pointerup", g)), window.addEventListener("pointercancel", (g) => this._OnPointerEvent("pointercancel", g))) : (window.addEventListener("mousedown", (g) => {
          this._HandlePointerDownFocus(g);
          this._OnMouseEventAsPointer("pointerdown", g);
        }), window.addEventListener("mousemove", (g) => this._OnMouseEventAsPointer("pointermove", g)), window.addEventListener(
          "mouseup",
          (g) => this._OnMouseEventAsPointer("pointerup", g)
        ...
        window.addEventListener("pointerup", f, true);
        window.addEventListener(
          "touchend",
          f,
          true
        ...
        window.addEventListener("gamepadconnected", f, true);
        this._iRuntime.IsAndroid() && !this._iRuntime.IsAndroidWebView() && navigator.virtualKeyboard && (navigator.virtualKeyboard.overlaysContent = true, navigator.virtualKeyboard.addEventListener("geometrychange", () => {
      ...
      _OnWindowResize() {
        const f = this._GetWindowInnerWidth(), g = this._GetWindowInnerHeight();
        ...
        this._PostRuntimeEvent("window-resize", { innerWidth: f, innerHeight: g, devicePixelRatio: window.devicePixelRatio, isFullscreen: d.IsDocumentFullscreen() });
      ...
      _OnSimulatedResize(f, g, c) {
        const h = this._GetWindowInnerWidth(), l = this._GetWindowInnerHeight();
        this._simulatedResizeTimerId = -1;
        h != f || l != g ? this._PostRuntimeEvent(
          "window-resize",
          { innerWidth: h, innerHeight: l, devicePixelRatio: window.devicePixelRatio, isFullscreen: d.IsDocumentFullscreen() }
      ...
      _OnFullscreenChange() {
        ...
        this.PostToRuntime("fullscreenchange", { isFullscreen: f, innerWidth: this._GetWindowInnerWidth(), innerHeight: this._GetWindowInnerHeight() });
      }
      _OnFullscreenError(f) {
        console.warn(
          "[Construct 3] Fullscreen request failed: ",
          f
        );
        this.PostToRuntime("fullscreenerror", { isFullscreen: d.IsDocumentFullscreen(), innerWidth: this._GetWindowInnerWidth(), innerHeight: this._GetWindowInnerHeight() });
      }
      ...
      _OnMouseWheelEvent(f, g) {
        this.PostToRuntime(f, { clientX: g.clientX, clientY: g.clientY, pageX: g.pageX, pageY: g.pageY, deltaX: g.deltaX, deltaY: g.deltaY, deltaZ: g.deltaZ, deltaMode: g.deltaMode, timeStamp: g.timeStamp }, e);
      }
      _OnMouseEvent(f, g, c) {
        a(g) || this._PostToRuntimeMaybeSync(f, { button: g.button, buttons: g.buttons, clientX: g.clientX, clientY: g.clientY, pageX: g.pageX, pageY: g.pageY, movementX: g.movementX || 0, movementY: g.movementY || 0, timeStamp: g.timeStamp }, c);
      }
      _OnMouseEventAsPointer(f, g) {
        if (!a(g)) {
          ...
          this._PostToRuntimeMaybeSync(f, { pointerId: 1, pointerType: "mouse", button: g.button, buttons: g.buttons, lastButtons: c, clientX: g.clientX, clientY: g.clientY, pageX: g.pageX, pageY: g.pageY, movementX: g.movementX || 0, movementY: g.movementY || 0, width: 0, height: 0, pressure: 0, tangentialPressure: 0, tiltX: 0, tiltY: 0, twist: 0, timeStamp: g.timeStamp }, e);
      ...
      _AttachDeviceOrientationEvent() {
        this._attachedDeviceOrientationEvent || (this._attachedDeviceOrientationEvent = true, window.addEventListener("deviceorientation", (f) => this._OnDeviceOrientation(f)), window.addEventListener("deviceorientationabsolute", (f) => this._OnDeviceOrientationAbsolute(f)));
      }
      _AttachDeviceMotionEvent() {
        this._attachedDeviceMotionEvent || (this._attachedDeviceMotionEvent = true, window.addEventListener("devicemotion", (f) => this._OnDeviceMotion(f)));
      }
      _OnDeviceOrientation(f) {
        this.PostToRuntime(
          "deviceorientation",
          { absolute: !!f.absolute, alpha: f.alpha || 0, beta: f.beta || 0, gamma: f.gamma || 0, timeStamp: f.timeStamp, webkitCompassHeading: f.webkitCompassHeading, webkitCompassAccuracy: f.webkitCompassAccuracy },
          e
        );
      }
      _OnDeviceOrientationAbsolute(f) {
        this.PostToRuntime("deviceorientationabsolute", { absolute: !!f.absolute, alpha: f.alpha || 0, beta: f.beta || 0, gamma: f.gamma || 0, timeStamp: f.timeStamp }, e);
      }
      _OnDeviceMotion(f) {
        let g = null;
        ...
        h && (c = { x: h.x || 0, y: h.y || 0, z: h.z || 0 });
        h = null;
        const l = f.rotationRate;
        l && (h = { alpha: l.alpha || 0, beta: l.beta || 0, gamma: l.gamma || 0 });
        this.PostToRuntime("devicemotion", { acceleration: g, accelerationIncludingGravity: c, rotationRate: h, interval: f.interval, timeStamp: f.timeStamp }, e);
      }
      _OnUpdateCanvasSize(f) {
        var g = this.GetRuntimeInterface();
        g.IsExportingToVideo() || (g = g.GetCanvas(), g.style.width = f.styleWidth + "px", g.style.height = f.styleHeight + "px", g.style.marginLeft = f.marginLeft + "px", g.style.marginTop = f.marginTop + "px", this._isFirstSizeUpdate && (g.style.display = "", this._isFirstSizeUpdate = false));
      }
      _OnInvokeDownload(f) {
        const g = f.url;
        f = f.filename;
        const c = document.createElement("a"), h = document.body;
        c.textContent = f;
        c.href = g;
        c.download = f;
        h.appendChild(c);
      ...
      _PlayPendingMedia() {
        ...
        if (!this._isSilent) for (const g of f) (f = g.play()) && f.catch((c) => {
      ...
      _OnWrapperMessage(f) {
        "entered-fullscreen" === f ? (d._SetWrapperIsFullscreenFlag(true), this._OnFullscreenChange()) : "exited-fullscreen" === f ? (d._SetWrapperIsFullscreenFlag(false), this._OnFullscreenChange()) : console.warn("Unknown wrapper message: ", f);
      }
      ...
      _OnExportedToVideo(f) {
        window.c3_postToMessagePort({ type: "exported-video", blob: f.blob, time: f.time });
      }
    });
  }
  self.JobSchedulerDOM = class {
    ...
    async Init() {
      ...
      this._dispatchWorker = await this._runtimeInterface.CreateWorker(d, this._baseUrl, { name: "DispatchWorker" });
    ...
    async _CreateJobWorker() {
      ...
      const b = new MessageChannel(), e = new MessageChannel();
      this._dispatchWorker.postMessage({
        type: "_addJobWorker",
        port: b.port1
      }, [b.port1]);
      a.postMessage({ type: "init", number: d, "dispatch-port": b.port2, "output-port": e.port2 }, [b.port2, e.port2]);
  ...
  window.C3_IsSupported && (window.c3_runtimeInterface = new self.RuntimeInterface({ useWorker: false, workerMainUrl: "workermain.js", engineScripts: ["scripts/c3runtime.js"], projectScripts: [], mainProjectScript: "", scriptFolder: "scripts/", workerDependencyScripts: [], exportType: "html5" }));
  self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    async _RequestOrientationPermission() {
      ...
      try {
        ...
        return console.warn("[Touch] Failed to request orientation permission: ", d), false;
      }
    }
    async _RequestMotionPermission() {
      ...
      try {
        ...
        return console.warn("[Touch] Failed to request motion permission: ", d), false;
      }
    }
  });
  self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _OnLockOrientation(d) {
      ...
      if (screen.orientation && screen.orientation.lock) screen.orientation.lock(d).catch((a) => console.warn("[Construct 3] Failed to lock orientation: ", a));
        ...
        console.warn(
          "[Construct 3] Failed to lock orientation: ",
          a
        );
      }
    }
    ...
    _OnRequestFullscreen(d) {
      if ("windows-webview2" === this._exportType || "macos-wkwebview" === this._exportType) self.RuntimeInterface._SetWrapperIsFullscreenFlag(true), this._iRuntime._SendWrapperMessage({ type: "set-fullscreen", fullscreen: true });
      else {
        const a = { navigationUI: "auto" };
        d = d.navUI;
        1 === d ? a.navigationUI = "hide" : 2 === d && (a.navigationUI = "show");
        d = document.documentElement;
        d.requestFullscreen ? d.requestFullscreen(a) : d.mozRequestFullScreen ? d.mozRequestFullScreen(a) : d.msRequestFullscreen ? d.msRequestFullscreen(a) : d.webkitRequestFullScreen && ("undefined" !== typeof Element.ALLOW_KEYBOARD_INPUT ? d.webkitRequestFullScreen(Element.ALLOW_KEYBOARD_INPUT) : d.webkitRequestFullScreen());
      }
    }
    _OnExitFullscreen() {
      "windows-webview2" === this._exportType || "macos-wkwebview" === this._exportType ? (self.RuntimeInterface._SetWrapperIsFullscreenFlag(false), this._iRuntime._SendWrapperMessage({ type: "set-fullscreen", fullscreen: false })) : document.exitFullscreen ? document.exitFullscreen() : document.mozCancelFullScreen ? document.mozCancelFullScreen() : document.msExitFullscreen ? document.msExitFullscreen() : document.webkitCancelFullScreen && document.webkitCancelFullScreen();
    }
    _OnSetHash(d) {
  ...
  self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(d) {
      super(d, "mouse");
      this.AddRuntimeMessageHandlers([["cursor", (a) => this._OnChangeCursorStyle(a)], ["request-pointer-lock", () => this._OnRequestPointerLock()], ["release-pointer-lock", () => this._OnReleasePointerLock()]]);
  ...
  });
  {
    ...
    self.AudioDOMHandler = class extends self.DOMHandler {
      constructor(a) {
        ...
        this._hasAttachedUnblockEvents = this._hasUnblocked = false;
        ...
        this._pendingTags = /* @__PURE__ */ new Map();
      ...
      async _CreateAudioContext(a) {
        a.isiOSCordova && (this._playMusicAsSound = true);
        ...
        try {
          await Promise.all(a.preloadList.map((e) => this._GetAudioBuffer(e.originalUrl, e.url, e.type, false)));
        } catch (e) {
      ...
      _AttachUnblockEvents() {
        this._hasAttachedUnblockEvents || (this._hasUnblocked = false, window.addEventListener("pointerup", this._unblockFunc, true), window.addEventListener("touchend", this._unblockFunc, true), window.addEventListener("click", this._unblockFunc, true), window.addEventListener(
          "keydown",
          this._unblockFunc,
          true
      ...
      _DetachUnblockEvents() {
        this._hasAttachedUnblockEvents && (this._hasUnblocked = true, window.removeEventListener("pointerup", this._unblockFunc, true), window.removeEventListener("touchend", this._unblockFunc, true), window.removeEventListener("click", this._unblockFunc, true), window.removeEventListener("keydown", this._unblockFunc, true), this._hasAttachedUnblockEvents = false);
      }
      _UnblockAudioContext() {
        if (!this._hasUnblocked) {
          var a = this._audioContext;
          "suspended" === a.state && a.resume && a.resume();
          var b = a.createBuffer(1, 220, 22050), e = a.createBufferSource();
          e.buffer = b;
      ...
      *audioInstancesByTag(a) {
      ...
      async _GetAudioInstance(a, b, e, k, m) {
        for (const p of this._audioInstances) if (p.GetUrl() === b && (p.CanBeRecycled() || m)) return p.SetTag(k), p;
      ...
      _AddPendingTag(a) {
        ...
        if (!b) {
          let e = null;
          b = { pendingCount: 0, promise: new Promise((k) => e = k), resolve: e };
      ...
      _MaybeStartTicking() {
        ...
        else for (const a of this._audioInstances) if (a.IsActive()) {
          this._StartTicking();
          break;
        }
      }
      Tick() {
        ...
        this.PostToRuntime("state", { tickCount: this._lastTickCount, audioInstances: b, analysers: [...this._analysers].map((e) => e.GetData()) });
      ...
      async _Play(a) {
        const b = a.originalUrl, e = a.url, k = a.type, m = a.isMusic, p = a.tag, u = a.isLooping, z = a.vol, B = a.pos, v = a.panning;
        let w = a.off;
        ...
        try {
          this._lastAudioInstance = await this._GetAudioInstance(b, e, k, p, m), v ? (this._lastAudioInstance.SetPannerEnabled(true), this._lastAudioInstance.SetPan(v.x, v.y, v.angle, v.innerAngle, v.outerAngle, v.outerGain), v.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(v.uid)) : this._lastAudioInstance.SetPannerEnabled(false), this._lastAudioInstance.Play(u, z, B, w);
        } catch (A) {
      ...
      async _FadeVolume(a) {
        const b = a.tag, e = a.vol, k = a.duration;
        a = a.stopOnEnd;
      ...
      async _Preload(a) {
        const b = a.originalUrl, e = a.url, k = a.type;
        a = a.isMusic;
        try {
      ...
      async _Unload(a) {
        if (a = await this._GetAudioBuffer("", a.url, a.type, a.isMusic, true)) a.Release(), a = this._audioBuffers.indexOf(a), -1 !== a && this._audioBuffers.splice(a, 1);
      }
      _UnloadAll() {
      ...
      _OnTick(a) {
        ...
        (b = a.listenerPos) && this._audioContext.listener.setPosition(
          b[0],
          b[1],
          b[2]
        );
      ...
      async _AddEffect(a) {
        ...
        if ("filter" === b) k = new self.C3AudioFilterFX(this, ...k);
          ...
          b = null;
          try {
            b = await this._GetAudioBuffer(a.bufferOriginalUrl, a.bufferUrl, a.bufferType, false);
          } catch (m) {
            console.log("[Construct 3] Audio: error loading convolution: ", m);
            return;
          }
          k = new self.C3AudioConvolveFX(this, b.GetAudioBuffer(), ...k);
        ...
        else if ("compressor" === b) k = new self.C3AudioCompressorFX(this, ...k);
      ...
      _SetEffectParam(a) {
        const b = a.index, e = a.param, k = a.value, m = a.ramp, p = a.time;
      ...
      _PostUpdatedFxState() {
        this._isPendingPostFxState || (this._isPendingPostFxState = true, Promise.resolve().then(() => this._DoPostUpdatedFxState()));
      }
      ...
      async _OnLoadState(a) {
        ...
        if (3 !== b) for (var e of this._audioInstances) e.IsMusic() && 1 === b || (e.IsMusic() || 2 !== b) && e.Stop();
        for (const k of this._effects.values()) for (const m of k) m.Release();
        this._effects.clear();
        this._timeScale = a.timeScale;
        this._gameTime = a.gameTime;
        e = a.listenerPos;
        ...
        await Promise.all(e);
        await Promise.all(a.playing.map((k) => this._LoadAudioInstance(k, b)));
        this._MaybeStartTicking();
      }
      async _LoadAudioInstance(a, b) {
        if (3 !== b) {
          var e = a.bufferOriginalUrl, k = a.bufferUrl, m = a.bufferType, p = a.isMusic, u = a.tag, z = a.isLooping, B = a.volume, v = a.playbackTime;
          if (!p || 1 !== b) {
            if (p || 2 !== b) {
              b = null;
              try {
      ...
      static EqualsNoCase(a, b) {
        return a.length !== b.length ? false : a === b ? true : a.toLowerCase() === b.toLowerCase();
      }
      static ToDegrees(a) {
        return a * d;
      }
  ...
  self.C3AudioBuffer = class {
    ...
    static Create(d, a, b, e, k) {
      ...
      return !k || d.IsPlayMusicAsSound() || m ? new self.C3WebAudioBuffer(d, a, b, e, k, m) : new self.C3Html5AudioBuffer(
        d,
        a,
        b,
        e,
  ...
  self.C3Html5AudioBuffer = class extends self.C3AudioBuffer {
    ...
    _OnError(d) {
      ...
      this._loadReject && (this._loadState = "failed", this._loadReject(d), this._loadReject = this._loadResolve = null);
    }
    IsLoaded() {
      const d = 4 <= this._audioElem.readyState;
      d && (this._reachedCanPlayThrough = true);
  ...
  self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
    ...
    async _Fetch() {
      ...
      if ("cordova" === d.GetExportType() && d.IsRelativeURL(this._url) && d.IsFileProtocol()) this._audioData = await d.CordovaFetchLocalFileAsArrayBuffer(this._url);
      else {
    ...
    async _Load() {
      try {
        ...
        this._loadState = "failed", console.error(`[Construct 3] Failed to load audio '${this._url}': `, d);
      }
    }
    IsLoaded() {
  ...
  };
  {
    ...
    self.C3AudioInstance = class {
      constructor(a, b, e) {
        ...
        this._isLooping = this._resumeMe = this._isPaused = false;
        this._volume = 1;
        this._isMuted = false;
      ...
      SetPan(a, b, e, k, m, p) {
        this._isPannerEnabled && (this.SetPanXYA(a, b, e), a = self.AudioDOMHandler.ToDegrees, this._pannerNode.coneInnerAngle = a(k), this._pannerNode.coneOuterAngle = a(m), this._pannerNode.coneOuterGain = p);
      }
      SetPanXYA(a, b, e) {
      ...
      GetState() {
        return { aiid: this.GetAiId(), tag: this._tag, duration: this.GetDuration(), volume: this._volume, isPlaying: this.IsPlaying(), playbackTime: this.GetPlaybackTime(), playbackRate: this.GetPlaybackRate(), uid: this._instUid, bufferOriginalUrl: this.GetOriginalUrl(), bufferUrl: "", bufferType: this.GetContentType(), isMusic: this.IsMusic(), isLooping: this.IsLooping(), isMuted: this.IsMuted(), resumePosition: this.GetResumePosition(), pan: this.GetPanState() };
      }
      ...
      LoadPanState(a) {
        if (a) {
          ...
          this._instUid = a.uid;
        } else this.SetPannerEnabled(false);
      }
    };
  }
  self.C3Html5AudioInstance = class extends self.C3AudioInstance {
    ...
    CanBeRecycled() {
      return this._isStopped ? true : this.HasEnded();
    }
    GetPlaybackTime() {
    ...
    Play(d, a, b, e) {
      ...
      this._isPaused = this._isStopped = false;
      this._isLooping = d;
      this._playbackRate = 1;
    }
    Stop() {
    ...
    Resume() {
      !this._isPaused || this._isStopped || this.HasEnded() || (this._audioDomHandler.TryPlayMedia(this.GetAudioElement()), this._isPaused = false);
    }
    _UpdateMuted() {
    ...
    Seek(d) {
      if (!this._isStopped && !this.HasEnded()) try {
        ...
        console.warn(
          `[Construct 3] Error seeking audio to '${d}': `,
          a
        );
      }
    }
    ...
    SetSuspended(d) {
      d ? this.IsPlaying() ? (this.GetAudioElement().pause(), this._resumeMe = true) : this._resumeMe = false : this._resumeMe && (this._audioDomHandler.TryPlayMedia(this.GetAudioElement()), this._resumeMe = false);
    }
  };
  self.C3WebAudioInstance = class extends self.C3AudioInstance {
    ...
    _OnEnded(d) {
      this._isPaused || this._resumeMe || d.target !== this._activeSource || (this._isStopped = this._hasPlaybackEnded = true, this._instUid = -1, this._ReleaseBufferSource(), this._audioDomHandler.PostTrigger("ended", this._tag, this._aiId));
    }
    HasEnded() {
      return !this._isStopped && this._bufferSource && this._bufferSource.loop || this._isPaused ? false : this._hasPlaybackEnded;
    }
    CanBeRecycled() {
      return !this._bufferSource || this._isStopped ? true : this.HasEnded();
    }
    GetPlaybackTime() {
      let d;
      d = this._isPaused ? this._resumePosition : this._playFromSeekPos + (this.GetCurrentTime() - this._playStartTime) * this._playbackRate;
      this._isLooping || (d = Math.min(d, this.GetDuration()));
      return d;
    }
    Play(d, a, b, e) {
      ...
      this._isPaused = this._isStopped = this._hasPlaybackEnded = false;
      this._isLooping = d;
      this._playbackRate = 1;
    ...
    Pause() {
      this._isPaused || this._isStopped || this.HasEnded() || (this._resumePosition = this.GetPlaybackTime(), this._isLooping && (this._resumePosition %= this.GetDuration()), this._isPaused = true, this._bufferSource.stop(0));
    }
    Resume() {
    ...
    SetSuspended(d) {
      d ? this.IsPlaying() ? (this._resumeMe = true, this._resumePosition = this.GetPlaybackTime(), this._isLooping && (this._resumePosition %= this.GetDuration()), this._bufferSource.stop(0)) : this._resumeMe = false : this._resumeMe && (this._ReleaseBufferSource(), this._bufferSource = this.GetAudioContext().createBufferSource(), this._bufferSource.buffer = this._buffer.GetAudioBuffer(), this._bufferSource.connect(this._gainNode), this._activeSource = this._bufferSource, this._bufferSource.onended = this._onended_handler, this._bufferSource.loop = this._isLooping, this._UpdateVolume(), this._UpdatePlaybackRate(), this._bufferSource.start(0, this._resumePosition), this._playStartTime = this.GetCurrentTime(), this._playFromSeekPos = this._resumePosition, this._resumeMe = false);
    }
  ...
  };
  {
    ...
    self.C3AudioConvolveFX = class extends d {
      ...
      SetParam(a, b, e, k) {
        switch (a) {
          case 0:
            b = Math.max(Math.min(b / 100, 1), 0), this._params[1] = b, this.SetAudioParam(this._wetNode.gain, b, e, k), this.SetAudioParam(
              this._dryNode.gain,
              1 - b,
              e,
              k
    ...
    self.C3AudioDistortionFX = class extends d {
      ...
      SetParam(a, b, e, k) {
        switch (a) {
          case 0:
            b = Math.max(Math.min(b / 100, 1), 0), this._params[4] = b, this.SetAudioParam(this._wetNode.gain, b, e, k), this.SetAudioParam(this._dryNode.gain, 1 - b, e, k);
        }
      }
    };
